...
export async function createAccount(input) {
    try {
        ...
        const existingAccount = await getQuery('SELECT id FROM accounts WHERE code = ?', [input.code]);
        if (existingAccount) {
            return {
                success: false,
        ...
        if (input.parentId) {
            const parentAccount = await getQuery('SELECT id FROM accounts WHERE id = ?', [input.parentId]);
            if (!parentAccount) {
                return {
                    success: false,
        ...
        // Create the account
        const result = await runQuery('INSERT INTO accounts (code, name, type, parent_id) VALUES (?, ?, ?, ?)', [input.code, input.name, input.type, input.parentId || null]);
        // Fetch the created account
        const createdAccount = await getQuery('SELECT * FROM accounts WHERE id = ?', [result.lastInsertRowid]);
        if (!createdAccount) {
            return {
                success: false,
...
export async function updateAccount(input) {
    try {
        ...
        const existingAccount = await getQuery('SELECT * FROM accounts WHERE id = ?', [input.id]);
        if (!existingAccount) {
            return {
                success: false,
        ...
        if (input.code && input.code !== existingAccount.code) {
            const codeExists = await getQuery('SELECT id FROM accounts WHERE code = ? AND id != ?', [input.code, input.id]);
            if (codeExists) {
                return {
                    success: false,
                    message: 'Account code already exists',
        ...
        await runQuery(`UPDATE accounts SET ${updates.join(', ')} WHERE id = ?`, params);
        // Fetch the updated account
        const updatedAccount = await getQuery('SELECT * FROM accounts WHERE id = ?', [input.id]);
        return {
            success: true,
...
export async function getAccountById(input) {
    try {
        const account = await getQuery('SELECT * FROM accounts WHERE id = ?', [input.id]);
        if (!account) {
            return {
                success: false,
                message: 'Account not found',
...
export async function listAccounts(input) {
    try {
        ...
        sql += ' ORDER BY code';
        const accounts = await getAllQuery(sql, params);
        return {
            success: true,
...
async function getDescendantAccountIds(accountId) {
    ...
    while (queue.length > 0) {
        ...
        const children = await getAllQuery('SELECT id FROM accounts WHERE parent_id = ?', [currentId]);
...
export async function deleteAccount(input) {
    try {
        // Check if account exists
        const existingAccount = await getQuery('SELECT id FROM accounts WHERE id = ?', [input.id]);
        if (!existingAccount) {
            return {
                success: false,
                message: 'Account not found',
        ...
        const hasJournalLines = await getQuery(`SELECT COUNT(*) as count FROM journal_lines WHERE account_id IN (${descendantIds.map(() => '?').join(',')})`, descendantIds);
