 ...
 */
async function getTrialBalance(input) {
    try {
        let sql = `
      SELECT 
        a.id as account_id,
    ...
    `;
      ...
      ORDER BY a.code
    `;
        const rows = await (0, connection_1.getAllQuery)(sql, params);
 ...
 */
async function getLedger(input) {
    try {
        // First, get account information
        const accountRow = await (0, connection_1.getQuery)('SELECT * FROM accounts WHERE id = ?', [input.accountId]);
        if (!accountRow) {
            return {
                success: false,
    ...
    `;
        ...
        if (input.startDate) {
            const openingRow = await (0, connection_1.getQuery)(`
        SELECT 
 ...
 */
async function getBalanceSheet(input) {
    try {
        const asOfDate = input.asOfDate || luxon_1.DateTime.now().toISODate();
        let sql = `
      SELECT 
        a.id as account_id,
        a.code as account_code,
...
async function getProfitLoss(input) {
    try {
      ...
      ORDER BY a.type, a.code
    `;
        const rows = await (0, connection_1.getAllQuery)(sql, [input.startDate, input.endDate]);
        // Group by account type
        const incomeItems = [];
        const expenseItems = [];
        for (const row of rows) {
