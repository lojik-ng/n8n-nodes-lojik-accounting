...
export async function getLedger(input) {
    try {
        ...
        const accountRow = await getQuery('SELECT * FROM accounts WHERE id = ?', [input.accountId]);
        if (!accountRow) {
            return {
                success: false,
    ...
    `;
        ...
        if (input.startDate) {
            const openingRow = await getQuery(`
        SELECT 
 ...
 */
export async function getProfitLoss(input) {
    try {
        let sql = `
      SELECT 
        a.id as account_id,
      ...
      ORDER BY a.type, a.code
    `;
        const rows = await getAllQuery(sql, [input.startDate, input.endDate]);
        // Group by account type
        const incomeItems = [];
        const expenseItems = [];
        for (const row of rows) {
