...
async function isDateLocked(date) {
    const lock = await getQuery('SELECT through_date FROM period_locks ORDER BY through_date DESC LIMIT 1');
    if (!lock) {
        return false;
    }
...
async function validateAccountsExist(accountIds) {
    ...
    const existingAccounts = await getAllQuery(`SELECT id FROM accounts WHERE id IN (${placeholders})`, uniqueIds);
    ...
    try {
        ...
        await runTransaction([
            async () => {
                ...
                const entryResult = await runQuery('INSERT INTO journal_entries (date, description, reference) VALUES (?, ?, ?)', [input.date, input.description || null, input.reference || null]);
                const entryId = entryResult.lastInsertRowid;
                // Fetch the created entry
                const entryRow = await getQuery('SELECT * FROM journal_entries WHERE id = ?', [entryId]);
                createdEntry = mapRowToJournalEntry(entryRow);
                // Create journal lines
                for (const line of input.lines) {
                    const lineResult = await runQuery('INSERT INTO journal_lines (journal_entry_id, account_id, debit, credit) VALUES (?, ?, ?, ?)', [entryId, line.accountId, line.debit || 0, line.credit || 0]);
                    const lineRow = await getQuery('SELECT * FROM journal_lines WHERE id = ?', [lineResult.lastInsertRowid]);
...
export async function deleteJournalEntry(input) {
    try {
        ...
        const existingEntry = await getQuery('SELECT id, date FROM journal_entries WHERE id = ?', [input.id]);
        if (!existingEntry) {
            return {
                success: false,
...
export async function getJournalEntryById(input) {
    try {
        ...
        const entryRow = await getQuery('SELECT * FROM journal_entries WHERE id = ?', [input.id]);
        if (!entryRow) {
            return {
                success: false,
                message: 'Journal entry not found',
        ...
        const lineRows = await getAllQuery('SELECT * FROM journal_lines WHERE journal_entry_id = ? ORDER BY id', [input.id]);
        return {
            success: true,
            data: {
 ...
 */
export async function searchJournalEntries(input) {
    try {
        ...
        const entries = await getAllQuery(sql, params);
        return {
            success: true,
 ...
 */
export async function closePeriod(throughDate) {
    try {
        ...
        const existingLock = await getQuery('SELECT through_date FROM period_locks WHERE through_date >= ? ORDER BY through_date DESC LIMIT 1', [throughDate]);
        if (existingLock) {
            return {
                success: false,
        ...
        await runQuery('INSERT INTO period_locks (through_date) VALUES (?)', [throughDate]);
        return {
            success: true,
            data: { locked: true },
        };
