...
export async function getTrialBalance(db) {
    const rows = await allAsync(db, `SELECT a.id, a.code, a.name, a.type, a.parent_id, a.created_at,
...
export async function getLedger(db, input) {
    const account = (await allAsync(db, `SELECT * FROM accounts WHERE id = ?`, [
        input.accountId,
    ]))[0];
    if (!account)
        throw new Error('Account not found');
    const conditions = ['l.account_id = ?'];
    ...
    const rows = await allAsync(db, `SELECT l.id as line_id, l.journal_entry_id, l.debit, l.credit,
            e.date, e.description, e.reference, e.created_at
     FROM journal_lines l
     JOIN journal_entries e ON e.id = l.journal_entry_id
     ${where}
     ORDER BY e.date DESC, e.id DESC, l.id DESC`, params);
    let running = 0;
...
export async function getBalanceSheet(db) {
    const tb = await getTrialBalance(db);
    const pickWithSubtotals = (type) => {
        ...
        const parentIdToSubtotal = new Map();
        ...
        return { type: type, accounts, parentSubtotals };
    };
    return {
...
export async function getProfitLoss(db, input) {
    const conditions = [];
    const params = [];
    if (input?.startDate) {
        conditions.push('e.date >= ?');
    ...
    const rows = await allAsync(db, `SELECT a.id, a.code, a.name, a.type,
            IFNULL(SUM(l.debit),0) as totalDebit,
            IFNULL(SUM(l.credit),0) as totalCredit
     FROM accounts a
     LEFT JOIN journal_lines l ON l.account_id = a.id
